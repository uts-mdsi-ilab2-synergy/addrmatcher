...
class GeoMatcher:
    ...
    def get_map_data_by_address(
        ...
        img =  self._get_static_map_by_coord(geocode_dict['LATITUDE'], geocode_dict['LONGITUDE'], apikey)
    ...
    def _get_geocode_by_address(
        ...
        if not address_df: return
        addr_list = address_df.get('FULL_ADDRESS')
        if not addr_list: return 
        addr = addr_list[0]
        local = fs.LocalFileSystem()
        geocode_dict = {k:v[0] for k, v in pq.read_table(
            self._filename,
