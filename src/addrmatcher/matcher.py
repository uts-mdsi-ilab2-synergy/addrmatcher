...
class GeoMatcher:
    ...
    __slot__ = ('_hierarchy', '_file_location','_index_data', '_filename','_street_code_dict')
    ...
    def __init__(self, hierarchy, file_location=''):
        ...
        if file_location.strip() == '':
            if os.path.isdir(os.path.join('data\\',self._hierarchy.name)):
                ...
                raise ValueError('Folder names that contain the index file can\'t be found: '+os.path.join('data\\',self._hierarchy.name))
        else:
            if os.path.isdir(file_location):
                self._file_location = file_location
            else:
                raise ValueError('Folder names that contain the index file can\'t be found: '+file_location)
        ...
        if not set(idx_columns).issubset(self._index_data.columns):
            raise ValueError('The required columns can\'t be found in the index file: '+str(set(idx_columns) - set(self._index_data.columns)))
        ...
        for file in self._filename:
            ...
            if not set(all_columns).issubset(pq_columns):
                raise ValueError('The required columns '+str(set(all_columns) - set(pq_columns))+' can\'t be found in the parquet file: '+file)
        
        self._street_code_dict = {'ALLY': 'ALLEY',
                             'ALY': 'ALLEY',
                             'ARC': 'ARCADE',
                             'AVE': 'AVENUE',
                             'AV': 'AVENUE',
    ...
    def _cleaning_match_with_index(self, no_number_address):
        ...
        match = re.search('\s((?:NSW|VIC|QLD|TAS|WA|SA|NT|ACT))\s([0-9]{4})$', no_number_address)
        if not match: return matched_df
        ...
        matched_postcode_state = self._index_data[(self._index_data['POSTCODE'] == postcode) & (self._index_data['STATE'] == state)]
        if matched_postcode_state.empty: return matched_df
        ...
        if not suburbs : return matched_df
        ...
        if not any(index): return matched_df
        ...
        match = re.search(f"(?:{'|'.join(suburbs)})\s+{state}\s+{postcode}", no_number_address) 
        if not match: return matched_df
        ...
        if street_name: # if street name does not exist in the data frame, suburbs maybe incorrect
            ...
            street_name = [street_name for street_name in unique_street_name_list if street_name in street_string]
            matched_df = matched_postcode_state[matched_postcode_state['STREET_NAME'].isin(street_name)] 
    ...
    def _cleaning_address(self, no_number_address):
        ...
        if matched_df.empty:
            ...
            match = re.search(f"(?:({'|'.join(matched_df['STREET_NAME'].unique())}))\s(.*)\s((:?{'|'.join(matched_df['LOCALITY_NAME'].unique())}))", 
                                no_number_address)
        
            if not match: 
                match = re.search(f"(?:({'|'.join(matched_df['STREET_NAME'].unique())}))\s(\S*)\s",no_number_address)
                if not match: return no_number_address
            ...
            if suburb:
                ...
                index = (matched_df['STREET_NAME'] == street_name) & (matched_df['LOCALITY_NAME'] == suburb)
                # If no exact match found, try to find matching string with Street name and Suburb
                index = index if any(index) else (matched_df['STREET_NAME'].str.contains(street_name)) & (matched_df['LOCALITY_NAME'].str.contains(suburb))    
                
                # if there is no matching suburbs found, use street name only to filter
                matched_df = matched_df[index] if any(index) else matched_df[(matched_df['STREET_NAME'].str.contains(street_name))]
            
            else:
                matched_df = matched_df[(matched_df['STREET_NAME'] == street_name)]
            
            if matched_df.empty: return no_number_address
            if matched_df.shape[0] == 1: return matched_df['ADDRESS'].iloc[0]   
            
            # Covert street code to long form postcode
            street_code = self._street_code_dict[street_code] if street_code in self._street_code_dict else street_code
    ...
    def get_region_by_address(
        ...
        if (self._index_data is not None) and (self._index_data.shape[0] > 0):
            ...
            clean_address_idx = self._index_data[self._index_data["ADDRESS"] == clean_address]
            ...
            if os.path.isfile(self._file_location+"\\"+parquet_filename):
                ...
                address_parquet = pq.read_table(self._file_location+"\\"+parquet_filename,
                                                filesystem = fs.LocalFileSystem(),
                                                filters=[("IDX","=",parquet_idx)]).to_pandas()
                ...
                addresses = address_parquet[address_parquet["RATIO"] >= similarity_threshold * 100.0]
            ...
            raise ValueError("No index records found. Make sure the initiation process is succeeded")
